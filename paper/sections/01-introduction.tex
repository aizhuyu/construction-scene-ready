...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig01_scene_gap.pdf}
...
task binding even when each asset passes its own checks. The contract of
panel (d) is versioned: 30 frozen v0.2.0 baseline rules plus two
interface-fidelity rules added in v0.3.0 (32 in total,
\texttt{docs/scene-readiness-rules.csv}).}
\label{fig:scene-gap}
\end{figure*}
...
discovered only through failed simulation runs. The central research question
is which building information supports a robotic assembly task in simulation,
and how its absence, error, or imprecision changes the outcome. IFC4.3
entities are first translated into a construction scene intermediate
representation that retains component identity, work zones, assembly
interfaces, construction states, tasks, and property provenance. The
representation is then compiled into a global navigation layer, one or more
work-zone payloads, and task-specific interaction layers. This organization
permits OpenUSD to load detailed geometry only where the active task requires
it, while references preserve the connection to the same engineering objects
across layers.
...
layers. The central experiment then measures the downstream consequence of
each information family on a humanoid bolt-insertion task executed by a frozen
reinforcement-learning policy, and uses the measured consequences to
recalibrate the contract itself. In this way, the paper links an
information-model claim---that a scene satisfies an explicit construction
contract---to an operational claim---that the composed world supports the
intended robotic task.
