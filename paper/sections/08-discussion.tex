...
success from 85.5\% to exactly zero under both seed partitions. These features
...
semantic label is far less harmful than a \emph{wrong} one (13.0\% vs.\ 0.0\%):
...
careless unit handling can introduce---collapsed success from 85.5\% to
33.5\%, and a 0.25\,mm clearance is nearly fatal (5.5\%).
...
the 12\,mm alignment tolerance of the success criterion is much looser than
...
in absolute terms.} A 10\,mm shift of the entire connection node was largely
absorbed by closed-loop perception at moderate power ($-3.5$\,pp, n.s.\ at
$n$=200), whereas the same 10\,mm between the certified hole position and the
observed member cost $9.5$\,pp ($p<0.001$). At full power ($n$=1000) both
effects become measurable yet remain separable (member $-7.0$\,pp, hole
$-7.9$\,pp, both $p<10^{-15}$), while 2 and 5\,mm hole errors stay inside the
executor's tolerance band. The scene model's coordinate guarantees should
