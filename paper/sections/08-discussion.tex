...
careless unit handling can introduce---collapsed success from 87.0\% to
32.5\%, and a 0.25\,mm clearance is nearly fatal (7.5\%).
...
so the threshold values should not be read as physical measurements; a
first contact-mechanics counterpart is provided by the compliant magnetic
gripper experiment of \Cref{sec:physical-insertion}. Second,
...
become consequential in any stage with forceful contact. A first step in
this direction is the magnetic-gripper experiment of
\Cref{sec:physical-insertion}, in which a physically held bolt executes the
insertion with measured contact forces; a full-contact ablation of the
physical family remains required before it can be ranked.
...
deviate by at most 3.5\,pp from the seed-42 values, and effect directions
match in every condition; all strong effects remain strongly significant.
The criticality ordering is therefore a property of the scene-model features,
not of a particular random draw of episodes.
...
86.4\%, 86.6\%, and 79.1\% success for 2, 5, and 10\,mm against an 87.0\%
baseline; interface clearance gives 33.5\%, 21.5\%, and 5.5\% for 0.75,
0.5, and 0.25\,mm. Monotone dose--response supports a
causal interpretation of the injected defect rather than threshold noise in
the success criterion.

Third, the power upgrade from 200 to 1000 paired episodes clarifies the
borderline band. At $n$=200 neither the 10\,mm member-pose shift nor the
5\,mm hole shift crosses significance; at $n$=1000 the member-pose shift is
strongly significant ($p=5\times10^{-16}$) while 2 and 5\,mm hole errors
remain inside the executor's tolerance band (n.s.). We accordingly report
effect sizes alongside significance and do not rank features on $p$-values
alone; non-significant results are reported as inconclusive at the available
power, not as evidence of no effect.
