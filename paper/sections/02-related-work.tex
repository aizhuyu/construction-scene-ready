...
requirements reference. Both operate at the level of individual entities and
their declared properties: an IDS facet can require that a plate carries a
fire rating, but it cannot express that a task target must resolve to a
component that owns the bound interface, that an active work zone's payload
must stay inside the scene package, or that a critical physical value must
carry an authoritative provenance record. The present contract is
complementary by design: bSDD-style dictionaries supply standardized
semantics, IDS-style facets supply per-entity requirements, and the scene
contract adds cross-reference closure, task binding, composition safety, and
evidence authority at the level of the composed scene. \Cref{sec:results}
further shows empirically that even a fully satisfied contract leaves
sub-tolerance geometric fidelity invisible, which motivates the candidate
rules registered there.
