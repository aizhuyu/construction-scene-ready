...
\begin{figure*}[t]
\centering
\plannedfigure{42mm}{Figure 2 production placeholder: overall architecture}{
Left-to-right architecture with six stages: IFC4.3, task, and robot inputs;
typed construction-scene intermediate representation; global, work-zone, and
interaction USD layers; asset- and scene-level deterministic validation;
whitelisted repair or human escalation; and CPU reports plus RTX task
execution. Label the exchanged artefacts (IFC GUID map, scene JSON, USD layers,
diagnostic JSON, audit log, and task-outcome record) on the connecting arrows.}{
Compiler modules, schemas, CLI entry points, generated artefacts, and the
experiment registry. Editable SVG master and embedded-font PDF.}
...
\begin{figure*}[t]
\centering
\plannedfigure{41mm}{Figure 3 production placeholder: typed scene representation}{
An entity--relation diagram containing Component, AssemblyInterface, WorkZone,
ConstructionState, Task, Robot, Tool, RobotCapability, and ProvenanceRecord.
Show implemented typed edges such as \texttt{has\_interface},
\texttt{located\_in}, \texttt{mates\_with}, \texttt{targets},
\texttt{uses\_tool}, and \texttt{supports\_property}. Add one enlarged
provenance card for a critical property with value, source, confidence, and
uncertainty interval.}{
IR schema, JSON examples, IFC extraction mappings, and provenance ledger.
Vector SVG/PDF generated from a versioned diagram specification.}
...
The intended OpenUSD composition is illustrated in
\Cref{fig:multiscale-composition}. The final inset will report the measured
working-set effect; it will not be populated until the RTX batch experiment has
been executed.

\begin{figure*}[t]
\centering
\plannedfigure{43mm}{Figure 4 production placeholder: task-activated multi-scale composition}{
Panels (a)--(c) depict the same building task at global-navigation,
active-work-zone, and interaction fidelity. Panel (d) shows the OpenUSD
composition tree with layers, references, and payload activation. Preserve a
single IFC GUID across scales. Reserve a compact inset for measured active prim
count, peak GPU memory, and load time under all-loaded and task-activated
conditions.}{
Generated USDA layers, stable GUID map, true scene thumbnails, and RTX resource
logs. Diagram as SVG; thumbnails from the simulator; final hybrid PDF.}
...
\begin{table*}[t]
\centering
\caption{Construction-scene readiness contract to be implemented and
evaluated. The final manuscript will report versioned rule identifiers and
...
\begin{figure*}[t]
\centering
\plannedfigure{43mm}{Figure 5 production placeholder: contract and repair loop}{
Left: the six contract families with representative typed diagnostics. Centre:
a diagnostic record containing rule, severity, prim path, evidence, permitted
tools, and escalation policy. Right: the state sequence Compile--Validate--
Diagnose--Select tool--Repair--Revalidate--Pass/Escalate. Visually separate
the language-model proposal from deterministic acceptance; show one safe repair
and one mandatory escalation.}{
Rule registry, diagnostic schema, tool registry, and audit-log examples.
Vector-only SVG/PDF.}
