...
rule. A candidate fix \(a\in\mathcal{A}(e)\) is \emph{submitted} only if it
resolves its target violation without introducing a new blocking one and
stays within its evidence bounds:
\begin{align}
  \mathrm{submit}(a,e) \iff {}& e\notin V(a(S),T,\mathcal{Q})\ \land\
  \nexists\, e'\in V(a(S),T,\mathcal{Q}):\nonumber\\
  & \mathrm{severity}(e')=\mathrm{critical}\ \land\ e'\neq e ,
\end{align}
so a no-op or a mutation that hides the original violation cannot be
accepted. The repaired scene is then \emph{accepted} only when the task's
blocking requirements are all satisfied:
\begin{equation}
  \mathrm{accept}(S') \iff \nexists\, e\in V(S',T,\mathcal{Q}):
  \mathrm{severity}(e)=\mathrm{critical}.
\end{equation}
